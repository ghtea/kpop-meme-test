...
import { Box, Button, Flex, Icon, Image, Text } from 'components/atoms';
...
const ResultPage: NextPage<ResultPageProps> = ({ testId, defaultScore }) => {
  ...
  const answerChoices = useMemo(() => {
    ...
    const newAnswerChoices = typeof rawAnswerChoices === 'string' ? rawAnswerChoices.split('-') : [];
    const questionsLength = testData?.questions.length || 10;

    return [...newAnswerChoices, ...Array(questionsLength).fill(undefined)]
  ...
  const onClickCopyTestLink = useCallback(() => {
    const testLink = window.location.href.replace(/\/result.*/, '');
    copy(testLink);
        ...
        {testId && score !== undefined && (
          <Flex className="justify-start h-full">
            ...
            <Flex className="px-12 mt-4">
              ...
              <Flex className="mt-3 animate-rising flex-row gap-2">
                <Button onClick={onClickCopyResultLink} className="flex flex-row justify-center items-center gap-1">
                  <Icon glyph="link" />
                  {'결과'}
                </Button>
                <Button onClick={onClickCopyTestLink} className="flex flex-row justify-center items-center gap-1">
                  <Icon glyph="link" />
                  {'테스트'}
                </Button>
              </Flex>
              <Flex className="mt-3 bg-green-100 animate-rising">
                <Button onClick={onClickAnotherTest}>{`\'${anotherTestName}\' 하러가기`}</Button>
              </Flex>
            </Flex>
          </Flex>
        )}
