...
const TestPage: NextPage<TestPageProps> = ({ testId }) => {
  ...
  useEffect(() => {
    const activityId = testId === "food" ? "kpex1" : "kpex2";

    window.location.href = `https://www.seilight.com/ko/arcade/kpex/${activityId}`;
  }, []);
        ...
        <Flex className="relative justify-center h-full">
          {testId && (
            <Box className="absolute mr-2 cursor-pointer">
              <label htmlFor="start-button">
                <Image
                  alt={"food-start"}
                  src={`/${testId}/start.png`}
                  width={375}
                  height={600}
                />
              </label>
